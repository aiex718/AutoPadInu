...
namespace PuzzDraSolver
{
	...
	//空の状態では?め1にしておき、これが64bit目に?達したら?まったと判定する(bit1 >> 63)
	//?ートの?はMSBでわかる
	//全?イ??イ?展開
	//すでに宣言して?る変?に代入しようとするとコ?パイ?エ?ーになるなんで
	...
	public:
		...
		void pop_back(int length)
		{
			...
			bit1 = max(1uLL, bit1);//0にならないように
			bit2 = max(1uLL, bit2);//0にならないように
		}
	...
	inline void _simplifyRouteSideStep(Route route, Route& ansRoute)
	{
		...
		if ((movement + 2) % 4 == route.top())//?の移動の90度反対(意味のない移動)
		{
			route.pop_back();//?の移動も飛ばす
	...
	//意味のない移動を削?します。1度では全ての意味のない移動が消せない可能性も?ります。
	//
