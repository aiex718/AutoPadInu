...
namespace PuzzDraSolver
{
	Route beamSearch(BOARD& board, int aheads, int length, int beamWeight, int startRow, int startColumn, int& outScore)
	{
		...
		//length = min(length, MAX_ROUTE_SIZE);//大きさを制限

		////auto at = _routes[aheads][0];
		//int loopCount = (length - 1) / _routes[aheads][0].size() + !!((length - 1) % aheads);//lengthまでは必ず探索できるようにに?ープの回?を?める
		...
		//				////追加する?ートの一回目の移動が意味のないもので?る場?はその?ートから始まるものを飛ばす
		//				////なんか精度?ちた。バグってる？
